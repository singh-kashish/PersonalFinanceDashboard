...
const SignupCard = ({setShowSignup}:SignupCardProp) =>{
    const {showError,register,handleSubmit,onSubmit,errors,signupMutation} = useSignupHook();
return (<>
        {/* Error alert   */}
          {showError && (
            <div
            ...
            >
              <div className="flex items-center gap-2 text-sm font-medium text-[#FB7185]">
                ...
                <span>We couldn’t sign you up</span>
              </div>
              <p className="text-xs leading-snug text-[#FECDD3] text-wrap">
                There seems to be an issue, please check errors below and if this keeps happening, reach out to us.
              </p>
            </div>
          )}
          ...
          <footer className="text-xs text-center space-y-1">
            <p>
              <span className="dark:text-[#94A3B8]">Have an account? </span>
              <span
