...
export const summaryController = asyncHandler(async (req,res) =>{
        const result = await summaryService(req.validated?.query as AnalyticsQueryInput,req.auth.userId);
        sendSuccess(res,200,result,'Received Summary');
})
//Category
export const categoryController = asyncHandler(async (req,res) =>{
        const result = await categoryService(req.validated?.query as AnalyticsQueryInput,req.auth.userId);
