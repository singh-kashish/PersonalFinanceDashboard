  ...
  const currentUserController = asyncHandler(async (
    ...
    res
      ...
      const {userId:id,email} = req.auth;
      const name = await getNameById(id);
      sendSuccess(res,200,{id,email,name},'User details')
  });
  ...
  asyncHandler(async(req,res)=>{
      ...
      const {accessToken,refreshToken:newRefreshToken} = await safeRefreshTokenTransaction(refreshToken);
      res.cookie(
        'refreshToken',
        newRefreshToken,
        refreshCookieOptions
